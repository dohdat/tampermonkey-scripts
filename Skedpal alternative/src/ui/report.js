...
export { getUniqueAvailabilityMinutes };
let reportRenderToken = 0;

function buildReportHorizonRange(settings) {
  const horizonDays =
...
export function getTimeMapUsageRows(tasks = [], timeMaps = [], settings = state.settingsCache) {
  const { horizonStart, horizonEnd } = buildReportHorizonRange(settings);
  const scheduledIntervalsByTimeMap =
...
function buildTimeMapUsageSummary(uniqueAvailabilityMinutes, horizonStart, horizonEnd) {
  if (!Number.isFinite(uniqueAvailabilityMinutes)) {return null;}
  if (!horizonStart || !horizonEnd) {return null;}
  const msPerDay = HOURS_PER_DAY * MINUTES_PER_HOUR * MS_PER_MINUTE;
  const totalDays =
    Math.max(1, Math.round((horizonEnd.getTime() - horizonStart.getTime()) / msPerDay) + 1);
  const hoursPerDay = uniqueAvailabilityMinutes / (totalDays * MINUTES_PER_HOUR);
  const hoursLabel = formatHoursPerDay(hoursPerDay);
  const summary = document.createElement("div");
  summary.className = "mt-2 text-xs text-slate-400";
  summary.setAttribute("data-test-skedpal", "report-timemap-unique-availability");
  summary.textContent =
    `Unique availability: ${hoursLabel}/day over ${totalDays} days (${uniqueAvailabilityMinutes} min)`;
  return summary;
}

function buildTimeMapUsageRow(row) {
