...
export function buildChildSubsectionInput(sub, sectionId, isNoSection) {
  ...
  childSubsectionInputWrap.innerHTML = `
        <input data-child-subsection-input="${sub.id}" data-test-skedpal="subsection-child-input" placeholder="Add subsection" class="w-full rounded-lg border border-slate-800 bg-slate-950/80 px-3 py-2 text-xs text-slate-100 placeholder:text-slate-500 focus:border-lime-400 focus:outline-none" />
        <button type="button" data-submit-child-subsection="${sub.id}" data-parent-section="${
          isNoSection ? "" : sectionId
        }" data-test-skedpal="subsection-child-submit" class="rounded-lg border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-200 hover:border-lime-400">Add</button>
      `;
...
export function buildSubsectionZone(
  ...
  subZone.className =
    "rounded-lg border border-dashed border-slate-700 bg-slate-900/40 px-2 py-2";
