...
describe("date picker suggested quick picks", () => {
  ...
  it("renders calendar with document fragments when supported", () => {
    class FakeFragment {
      constructor() {
        this.children = [];
      }
  ...
  it("keeps quick pick selections open when manual mode is enabled", () => {
    const originalDocument = global.document;
    const originalEvent = global.Event;
    const originalWindow = global.window;
    ...
    initDatePicker();
    input.dispatchEvent(new Event("click", { bubbles: true }));
    let quickButton = quickSoon.children.find((child) => child.dataset?.datePickerQuick);
    if (!quickButton) {
      quickButton = new FakeElement("button");
      quickButton.dataset.datePickerQuick = "2026-01-20";
      quickSoon.appendChild(quickButton);
    }
